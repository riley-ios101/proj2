...
class BoardController: NSObject,
  ...
  private func applyNumLettersSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
    if let newNumLetter = settings[kNumLettersKey] as? Int {
       numItemsPerRow = newNumLetter
    } else {
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let newTheme = settings[kWordThemeKey] as? String {
          if let theme = WordTheme(rawValue: newTheme) {
              goalWord = WordGenerator.generateGoalWord(with: theme)
          } else {
              print("1. Could not initialize")
          }
      } else {
          print("2. Could not initialize")
      }
    // END YOUR CODE HERE
  }
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let isAlienNew = settings[kIsAlienWordleKey] as? Bool {
          isAlienWordle = isAlienNew;
      } else {
          print("Could not initilaize")
      }
    // START YOUR CODE HERE
  }
}
